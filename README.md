...
A pull-down shade and unread badges on the home icons, fed by a single
notifications table that the apps write into.

## Setup

Run `sql/notifications.sql` once in the SQL Editor, after the app files
you use (`schema.sql`, and `instagrat.sql` / `calendar.sql` if you have
them). It only wires up triggers for tables that exist, so it is safe to
run whatever subset you installed.

## What raises a notification

- **Messages** — a new message in a thread notifies the other members.
  Repeats in the same thread collapse into one row that counts up.
- **Instagrat** — someone follows you or requests to, accepts your
  follow, comments on or likes your post; your post is approved; and (for
  the GM) a post is waiting in the queue.
- **Calendar** — a GM event notifies everyone.

## The shade

Pull down from the top of any screen, tap the slim tab at the very top,
or tap the carrier bar. The shade lists everything newest first; unread
items are bright, read ones dimmed. Tap one to open the app it points at
(which marks it read), or **Clear all** to mark the lot read. A red dot
on the tab means something is unread.

## The badges

Each home icon shows its own unread count — messages, Instagrat, calendar
— straight from the notifications table, so a number appears when
something lands and clears when you have read it. Everything updates live,
no reload.

## Notes

- Notifications are per player and enforced by row-level security: a
  query only ever returns your own, and nothing but the trigger functions
  can create them, so none can be faked.
